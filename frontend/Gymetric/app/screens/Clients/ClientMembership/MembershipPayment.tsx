...
const MembershipPayment: FC<Props> = ({ form, handleForm, selectedMembership }) => {
    ...
    return (
        <View style={{ marginTop: 15 }}>
            ...
            <View>
                <View style={[$styles.card, { padding: spacing.md, marginVertical: spacing.md }]}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'space-between' }}>
                    {['Cash', 'UPI', 'Card', 'Transfer'].map((type: string, index: number) => (
                        <Pressable key={index} style={[$styles.card, { marginVertical: spacing.xs, width: '48%', alignItems: 'center', backgroundColor: form.method === type ? colors.surface : colors.surface, borderWidth: form.method === type ? 2 : 1, borderColor: form.method === type ? colors.tint : colors.border, padding: spacing.md }]} onPress={() => { handleForm('method', type) }}>
                ...
                <View style={[$styles.card, { padding: spacing.md, marginTop: spacing.md }, $styles.flexRow]}>
                    <View>
